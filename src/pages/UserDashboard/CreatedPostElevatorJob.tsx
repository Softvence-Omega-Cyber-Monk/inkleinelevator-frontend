...
export default function CreatedPostElevatorJob() {
  ...
  const { data: jobData, isLoading: isLoadingJob,refetch: refetchJob } = useGetSingleJobByIdQuery(jobId || "", {
    skip: !jobId,
  });
    ...
    if (isEditMode && jobId) {
      ...
      formData.photos?.forEach((photo: any) => {
        if (photo.isExisting && photo.url) {
          form.append("existingPhotos", photo.url);
      ...
      // Handle documents - append existing and new
      formData.documents?.forEach((doc: any) => {
        if (doc.isExisting && doc.url) {
          form.append("existingDocuments", doc.url);
      ...
      try {
        ...
        const errorMessage = err?.data?.message || err?.data?.error || err?.message || "Failed to update job";
