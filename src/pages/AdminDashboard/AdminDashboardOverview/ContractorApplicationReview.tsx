  ...
  export default function ContractorApplicationReview({data, close}) {

    const [verificationStatus, setVerificationStatus] = useState<'verified' | 'rejected' | null>(null);
    const [rejectionReason, setRejectionReason] = useState('');
    
    // Initialize the mutation hook
    const [verifyContractor, { isLoading, isSuccess, isError, error }] = useVerifyUserStatusMutation();


    console.log(data)

    const handleVerify = async () => {
      console.log('handle_veriry')
      try {
        // Call the mutation
        const result = await verifyContractor({id: data.userId, status: "VERIFID" }).unwrap(); // unwrap() gives you the actual response or throws an error
      ...
      <div className="fixed z-9999 w-full h-full top-0 left-0 bg-black/60 backdrop-blur-sm">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-[600px] max-h-[648px] rounded-lg p-6 bg-white overflow-y-auto">
            ...
            <div className="bg-gray-100 rounded-xl">
              <ul className="p-5 space-y-[16px]">
                <li className="flex justify-between">
                  <span>Contractor:</span> <span className="font-semibold text-base">{data.companyName }</span>
                </li>

                <li className="flex justify-between">
                  <span>Email:</span> <span className="font-semibold text-base">{data.email}</span>
                </li>

                <li className="flex justify-between">
                  <span>License Number:</span> <span className="font-semibold text-base">license Number</span>
                </li>

                <li className="flex justify-between">
                  <span>Applied:</span> <span className="font-semibold text-base">{formatToYMD(data.createdAt)}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-3 mt-3">
              ...
              <div className="w-full rounded-lg border border-gray-200 p-5 flex items-center gap-4">
                ...
                {/* Optional: Add download button for insurance if you have the link */}
                {/* <a 
                  className="ml-auto" 
                  href={data.insuranceInfo} 
                  download="insurance_cert.pdf"
                  target="_blank" 
                  rel="noopener noreferrer"
                >
                  <img src="/icons/download_icon.svg" alt="Download" />
          ...
          <div className="flex gap-6 mt-5 flex-row-reverse">
            <button className="rounded-[6px] py-2 px-4 bg-black text-white" onClick={handleVerify} disabled={isLoading}>Verify</button>
            <button className="rounded-[6px] py-2 px-4 border border-gray-300 text-gray-700 hover:bg-gray-50" onClick={close} disabled={isLoading}>Cancel</button>
          </div>

          <button onClick={close} disabled={isLoading} className="w-4 h-4 absolute top-[30px] right-[30px]">
