...
export default function ContractorApproval() {
    ...
    return (
        <>
            <div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200">
                ...
                <div className="space-y-4">
                    {constructorApprovalList.data.map((job, index) => (
                        ...
                        <button onClick={()=>onReviewClick(job)} className="w-full sm:w-auto px-4 py-2 bg-slate-900 text-white text-sm rounded hover:bg-slate-800">
                            Review
                        </button>
                        </div>  
                    ))}
            ...
            { modal.isOpen && <ContractorApplicationReview key={modal?.data?.userId} data={modal.data} close ={closeModal}/>}
        </>
